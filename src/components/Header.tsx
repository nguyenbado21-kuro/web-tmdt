        ...
        <div className="flex items-center justify-between h-14 sm:h-16 gap-2 sm:gap-4 w-full">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <Link
              ...
              className={`relative text-sm font-medium transition-colors py-2 ${isActivePath('/')
                  ? 'text-[#3e4299]'
                  : 'text-gray-700 hover:text-[#3e4299]'
                }`}
            >
              TRANG CHỦ
              {isActivePath('/') && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#3e4299] animate-slide-underline"></div>
              )}
            </Link>
                  ...
                  <div className="bg-white rounded-xl shadow-lg border border-gray-100 py-2">
                    {categories.map((cat: Category) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-[#eeeef0] hover:text-[#3e4299] transition-colors"
                      >
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <Link to="/cart" className="relative p-2 hover:bg-gray-100 rounded-full transition-colors">
              <svg className="w-5 h-5 sm:w-6 sm:h-6 text-[#3e4299]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        ...
        <div className="fixed inset-0 z-[9999] flex">
          ...
          >
            ...
            <nav className="flex flex-col gap-1 px-3 py-4 flex-1">
              <Link to="/" className={`text-sm font-medium py-2.5 px-3 rounded-lg transition-colors ${isActivePath('/') ? 'text-[#3e4299] bg-[#eeeef0]' : 'text-gray-700 hover:text-[#3e4299] hover:bg-gray-50'}`} onClick={() => setMobileOpen(false)}>Trang chủ</Link>
              <Link to="/shop" className={`text-sm font-medium py-2.5 px-3 rounded-lg transition-colors ${isActivePath('/shop') || isActivePath('/product') ? 'text-[#3e4299] bg-[#eeeef0]' : 'text-gray-700 hover:text-[#3e4299] hover:bg-gray-50'}`} onClick={() => setMobileOpen(false)}>Sản phẩm</Link>
                ...
                <div className="pl-4 space-y-1 border-l-2 border-gray-100 ml-3">
                  {categories.map((cat: Category) => (
                    <button key={cat.id} onClick={() => { handleCategoryClick(cat.id.toString()); setMobileOpen(false); }}
                      className="block w-full text-left text-sm text-gray-500 py-2 px-3 rounded-lg hover:text-[#3e4299] hover:bg-gray-50 transition-colors">
                      {cat.name}
                    </button>
                  ))}
                </div>
              ...
              <Link to="/orders" className={`text-sm font-medium py-2.5 px-3 rounded-lg transition-colors ${isActivePath('/orders') ? 'text-[#3e4299] bg-[#eeeef0]' : 'text-gray-700 hover:text-[#3e4299] hover:bg-gray-50'}`} onClick={() => setMobileOpen(false)}>Đơn hàng</Link>
              <Link to="/promotions" className={`text-sm font-medium py-2.5 px-3 rounded-lg transition-colors ${isActivePath('/promotions') ? 'text-[#3e4299] bg-[#eeeef0]' : 'text-gray-700 hover:text-[#3e4299] hover:bg-gray-50'}`} onClick={() => setMobileOpen(false)}>Khuyến mãi</Link>
              <Link to="/cart" className={`text-sm font-medium py-2.5 px-3 rounded-lg transition-colors flex items-center justify-between ${isActivePath('/cart') ? 'text-[#3e4299] bg-[#eeeef0]' : 'text-gray-700 hover:text-[#3e4299] hover:bg-gray-50'}`} onClick={() => setMobileOpen(false)}>
                <span>Giỏ hàng</span>
                {totalItems > 0 && (
                  <span className="w-5 h-5 bg-[#3e4299] text-white text-xs font-bold rounded-full flex items-center justify-center">
                    {totalItems > 9 ? '9+' : totalItems}
                  </span>
                )}
              </Link>
            ...
            <div className="px-4 py-4 border-t border-gray-100">
              {isLoggedIn ? (
                <Link to="/profile" className="flex items-center gap-2 text-sm font-medium text-gray-700 py-2.5 px-3 rounded-lg hover:text-[#3e4299] hover:bg-gray-50 transition-colors" onClick={() => setMobileOpen(false)}>
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                  {userName || 'Tài khoản'}
                </Link>
              ) : (
                <Link to="/login" className="block text-center text-sm font-medium bg-[#3e4299] text-white py-2.5 px-4 rounded-lg hover:bg-[#32358a] transition-colors" onClick={() => setMobileOpen(false)}>Đăng nhập</Link>
              )}
            </div>
          </div>
        </div>,
