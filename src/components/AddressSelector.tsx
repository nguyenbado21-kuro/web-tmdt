...
interface AddressSelectorProps {
  ...
  onUpdateAddress: (address: Address) => Promise<{ success: boolean; error?: string }>;
  ...
  const handleSaveAddress = async (addressData: Omit<Address, 'id'> | Address) => {
    ...
    try {
      ...
      if ('id' in addressData && addressData.id) {
        ...
        result = await onUpdateAddress(addressData as Address);
        if (result.success) {
          ...
          setToast({ message: result.error || 'Không thể cập nhật địa chỉ', type: 'error' });
        }
      } else {
        // Add new address
        result = await onAddAddress(addressData as Omit<Address, 'id'>);
        if (result.success) {
          ...
          setToast({ message: result.error || 'Không thể thêm địa chỉ', type: 'error' });
        }
      }
    } catch (error) {
      console.error('Error saving address:', error);
      setToast({ message: 'Có lỗi xảy ra khi lưu địa chỉ', type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
                        ...
                        {!address.isDefault && (
                          <button
                            onClick={async (e) => {
                              e.stopPropagation();
                              setToast(null);
                              const result = await onDeleteAddress(address.id);
                              if (result.success) {
                                setToast({ message: 'Xóa địa chỉ thành công!', type: 'success' });
                              } else {
                                setToast({ message: result.error || 'Không thể xóa địa chỉ', type: 'error' });
                              }
                            }}
