@php
    $parcel = Arr::get($shipmentShipviettelpost, 'parcels.0');
    $distanceUnit = Arr::get($parcel, 'distance_unit');
    $massUnit = Arr::get($parcel, 'mass_unit');

    $url = route('ecommerce.shipments.shipviettelpost.transactions.create', $shipment->id);
    $isShowButton = true;
    if (!is_in_admin(true)) {
        if (is_plugin_active('marketplace')) {
            $url = route('marketplace.vendor.orders.shipviettelpost.transactions.create', $shipment->id);
        }
    ...
    <div class="row">
        <div class="col-12 my-3 text-center">
            <div>
                ...
                @endphp -->
                <div>
                    <small class="text-secondary">{{ trans('plugins/shipviettelpost::shipviettelpost.estimated_days', ['day' => $days]) }}</small>
                </div>
            </div>
        </div>
        <div class="col-12 my-2">
            @include('plugins/shipviettelpost::address')
        </div>

        <div class="col-12 my-2">
            @include('plugins/shipviettelpost::shipping-fee')
        </div>
        <div class="col-12 mt-2">
            <span class="fw-bold" style="font-size: 18px">{{ trans('plugins/shipviettelpost::shipviettelpost.parcel') }}</span>
            ...
            @if (now()->subHours(24)->gt($rateCreated))
                <div class="col-12 my-3">
                    ...
                    <small>
                        ...
                        <span>{{ trans('plugins/shipviettelpost::shipviettelpost.note_5') }}</span>
                    </small>
                    </div>
                    <button type="button" class="btn btn-primary get-new-rates" data-url="{{ route('ecommerce.shipments.shipviettelpost.rates', $shipment->id) }}">
                        {{ trans('plugins/shipviettelpost::shipviettelpost.recheck_rate') }}
                    </button>
                </div>
            @endif
        @endif
